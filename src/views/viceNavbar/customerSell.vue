            ...
            <div class="col-md" style="background-color:#DFF7FF " >
              ...
              <table class="table table-hover " >
                ...
                <tr style="background-color: #C3F0FF">
                  ...
                  <th style="width: 550px;">用户</th>
                  <!--                  <th style="width: 520px;">介绍</th>-->
                  <th style="width: 55px;">消费量</th>
                ...
                <tr v-for="(item,index) in list" :key="index" style="vertical-align:middle">
                  ...
                  <td style="vertical-align:middle">{{item.price.toString()}}</td>
                </tr>
                </tbody>
              </table>
    ...
    async getOrder(){
      ...
      await this.$axios.post("getOrders",this.$qs.stringify({id,t1,t2})).then((res)=>{
        ...
        res.data.forEach((item)=> {
          let bid = item.username;
          let flag = 0
          for (var i = 0; i < tmp.length; i++) {
            if (tmp[i].username == bid) {
              tmp[i].len++
              tmp[i].price += item.price
              // console.log(tmp[i].price)
              tmp[i].price = Math.floor(tmp[i].price * 100) / 100;
              tmp[i].price.toFixed(2)
              // console.log(tmp[i].price)
              flag = 1
            }
          }
